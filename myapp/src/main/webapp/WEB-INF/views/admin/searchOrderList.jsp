				  ...
				  <ul class="pagination">
				    <c:choose>
						<c:when test="${(pageMaker.startPage eq 1) && (pageMaker.criteria.pageNumber eq 1)}">
							<li class="page-item disabled"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=1&searchSelect=${searchSelect}&searchText=${searchText }">&laquo;</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=1&searchSelect=${searchSelect}&searchText=${searchText }">&laquo;</a></li>
						</c:otherwise>
					</c:choose>
					
					<c:choose>
						<c:when test="${pageMaker.startPage gt pageMaker.rangeSize}">
							<li class="page-item"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=${pageMaker.startPage-1}&searchSelect=${searchSelect}&searchText=${searchText }">&lt;</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item disabled"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=${pageMaker.startPage-1}&searchSelect=${searchSelect}&searchText=${searchText }">&lt;</a></li>
						</c:otherwise>
					</c:choose>
					...
					<c:forEach var="i" begin="${pageMaker.startPage }" end="${pageMaker.endPage }">
						<c:choose>
							<c:when test="${pageMaker.criteria.pageNumber eq i}">
								<li class="page-item active"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=${i }&searchSelect=${searchSelect}&searchText=${searchText }">${i }</a></li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=${i }&searchSelect=${searchSelect}&searchText=${searchText }">${i }</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<c:choose>
						<c:when test="${pageMaker.endPage lt pageMaker.totalPage}">
							<li class="page-item"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=${pageMaker.endPage+1}&searchSelect=${searchSelect}&searchText=${searchText }">&gt;</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item disabled"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=${pageMaker.endPage+1}&searchSelect=${searchSelect}&searchText=${searchText }">&gt;</a></li>
						</c:otherwise>
					</c:choose>
					
					<c:choose>
						<c:when test="${(pageMaker.endPage eq pageMaker.totalPage) && (pageMaker.totalPage eq pageMaker.criteria.pageNumber)}">
							<li class="page-item disabled"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=${pageMaker.totalPage}&searchSelect=${searchSelect}&searchText=${searchText }">&raquo;</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="${path }/admin/searchOrderList.do?pageNumber=${pageMaker.totalPage}&searchSelect=${searchSelect}&searchText=${searchText }">&raquo;</a></li>
						</c:otherwise>
					</c:choose>
				...
				<ul class="list list-group list-group-horizontal">
				  	<li class="list-group-item">
				  		<select class="form-select" id="searchSelect" name="searchSelect">
					  		...
					  		<c:if test='${searchSelect == "member_id"}'>
								<option>검색 항목</option>
								<option value="no">주문 번호</option>
								<option value="member_id" selected>주문자 ID</option>
								<option value="receiver_name">수령자</option>
								<option value="receiver_address">받는 사람 주소</option>
								<option value="status">주문 상태</option>
							</c:if>
